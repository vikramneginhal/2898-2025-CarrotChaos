# Memory Leak Diagnosis instructions
## Purpose

Steps for tracking down JVM memory leaks in the `2898-2025-CarrotChaos` robot codebase.
...
Does the team use desktop simulation or know how to use it?
...
- **Set size limits on Beaverlib utilities**: when using Beaverlib utilities, audit any `mutableListOf`/`mutableMapOf` fields inside singletons (`object` declarations). They persist for the entire JVM lifetime, so add explicit size limits or cleanup hooks when storing runtime data.
...
- Share the histogram top entries and telemetry trends with the mentors.
